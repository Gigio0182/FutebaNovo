...
Aplicacao para registrar atletas, jogos semanais, gols e assistencias, com ranking consolidado.

## Funcionalidades

- Cadastro de atletas.
- Cadastro de jogo com participantes.
- Registro de evento por jogo (`goal` ou `assist`).
- Ranking por atleta:
  - `goals` (gols)
...
- `public/index.html`: tela de registro (atletas, jogos, eventos).
- `public/ranking.html`: tela de ranking.
- `api/athletes.js`: API de atletas.
- `api/matches.js`: API de jogos.
- `api/events.js`: API de eventos.
...
- APIs exigem token valido no header `Authorization`.
